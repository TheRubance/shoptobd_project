...
router.post('/login', async (req, res) => {
    const { facebookId, name, email } = req.body;  // Simulating Facebook login payload

    console.log('Facebook Login Route Hit');
    console.log('Request Data:', { facebookId, name, email });
    ...
    try {
        ...
        // Generate JWT Token
        const token = jwt.sign({ userId: user.rows[0].id, authType: 'Facebook' }, SECRET_KEY, { expiresIn: '1h' });

        console.log('JWT Token Generated:', token);
        res.json({ message: 'Facebook login successful', token, user: user.rows[0] });
    } catch (error) {
